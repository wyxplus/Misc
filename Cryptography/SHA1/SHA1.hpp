...
/** 
 * @brief               把一个块进行 hash，是 SHA1 核心代码
 * @param digest        进行摘要过程的保存结果  
 * @param block         一个 512 bits 的块
 * @param transforms    已转换的块数    
 */
...
/** 
 * @brief                       将输入的明文分成 512 bits 为一个块
 * @param buffer                进行摘要的明文   
 * @param block[BLOCK_INTS]     一个 512 bits 的块
 */
